...
import positionwebservice.IPositionInfoChangeServiceService;
import positionwebservice.PositionService;
...
@Service("positionChangeSendService")
public class PositionChangeSendServiceImpl implements IPositionChangeSendService{
	private static final Logger logger = Logger.getLogger(PositionChangeSendServiceImpl.class);
	@Autowired
	private BaseLogRecordMapper baseLogRecordMapper;
	@Autowired
	private WspositioninfoMapper wspositioninfoMapper;
	/**
	 * 
	 * 描述：职位信息推送的方法
...
//		beginTime = StringUtil.strToDate(strTime);
		...
		try{
			...
			try {
				secondPositionInfoSendCondition(xmlString,baseLogId);//第二次发送的，成功或者是失败了
			} catch (Exception e) {
				e.printStackTrace();
				logger.info(e.getMessage());
			}
	...
	//正常情况下的信息的处理
	private Long regularPositionInfoCondition(String xmlString) throws Exception{
		Long  baseLogId = 0L;
		//模拟的接口:
//		IPositionChangeService positionChange = new PositionChangeServiceImpl();
//		String xmlStr = positionChange.QueryPositionInfoChangeFromHRToOA(xmlString);
		//调用esb接口的代码
		IPositionInfoChangeServiceService ie = new IPositionInfoChangeServiceService();
		PositionService service = ie.getPositionServicePort();
		String xmlStr = service.queryPositionInfoChangeFromHRToOA(xmlString);
		System.out.println("show:::::::" + xmlStr);
		if(XmlResolve.messageHeadValidate(xmlStr) ==null){//解析xml Body中的内容
			System.out.println("验证进入异常块了");
		}else if(XmlResolve.messageHeadValidate(xmlStr).equals("301")){
			//往baseLog表中添加一条信息
			//往组织对应的表中添加异常的信息
			//读取异常代码
			//不是4个零
			...
			List<Wspositioninfo> listPosition = CommonUtil.xmlStringToWspositioninfo(xmlString,xmlStr, baseLogId);//这段代码需要解析下
			//暂时先记录xmlString里面的信息（到时候需要将xmlString 更改为xmlStr)
			LogRecordUtil.addPositionInfoAll(listPosition, wspositioninfoMapper);	
	 ...
	 * @throws Exception
	 */
	private void secondPositionInfoSendCondition(String xmlString,Long baseLogId){//
		try{
			//模拟的接口:
//			IPositionChangeService positionChange = new PositionChangeServiceImpl();
//			String xmlStr = positionChange.QueryPositionInfoChangeFromHRToOA(xmlString);
			...
			if(XmlResolve.messageHeadValidate(xmlStr) ==null){//解析xml Body中的内容
				...
				//提取xmlStr中的异常的信息，组织相关的信息，这些信息是从xml的内容中解析出来的
//				List<Wspositioninfo> listPosition = CommonUtil.xmlStringToWspositioninfo(xmlStr, baseLogId);//这段代码需要解析下
//				//暂时先记录xmlString里面的信息（到时候需要将xmlString 更改为xmlStr)
//				LogRecordUtil.addPositionInfoAll(listPosition, wspositioninfoMapper);	
				List<Wspositioninfo> listPosition = CommonUtil.xmlStringToWspositioninfo(xmlString,xmlStr, baseLogId);//这段代码需要解析下
				LogRecordUtil.addPositionInfoAll(listPosition, wspositioninfoMapper);	
			...
			List<Wspositioninfo> list = CommonUtil.xmlStringToWspositioninfo(xmlString, baseLogId);
			LogRecordUtil.addPositionInfoAll(list, wspositioninfoMapper);
	...
	public void queryPositionInfoChangeFromHRToOAHand(String begin,String end,String fnumber){
		...
		//根据上次的最大的时间，查询出来对应的变动的信息
		String xmlString = MessageSendHandlerQuery.selectChangePositionInfoHandler(beginTime,dt,fnumber);
