...
<head>
	...
	<script type="text/javascript">
	  ...
	  function check(){
		  ...
		  //var reg1 = /(^\d+(?:\.\d{1})?$)/;
		  var reg1=/^([\d]+|([\d]+[.]?|[\d]+[.]?[\d]+))$/; // 1、以数字开头
		                                                   // 2、以数字开头 后面可以全部是数字
		                                                   // 3、以数字开头 中间可以出现 小数点一次
	...
	<body>
	<div class="tableForm">
	<div class="title">有限合伙人编辑</div>
	<input type="hidden" id="pid" value="${pid }">
	<form action="<%=basePath%>limitedpartner/limitedpartner!updatePartner?id=${partner.id}&pid=${pid}" method="post" onsubmit="return check();">
	<table>
	<tr>
	 <td class="t-title">姓名:</td>
	 <td><input class="t-text" type="text" name="partner.partner_name" id="name" value="${partner.partner_name }"/><span class="required">*</span></td>
	 <td class="t-title">性别:</td>
	 <td>
	 <input type="radio" id="1" name="partner.partner_sex" value="男" <c:if test="${partner.partner_sex=='男'}">checked="checked"</c:if> />男&nbsp;&nbsp;&nbsp;
	 <input type="radio" id="2" name="partner.partner_sex" value="女" <c:if test="${partner.partner_sex=='女'}">checked="checked"</c:if> />女 
	 </td>
	</tr>
	<tr>
	<td class="t-title">名族:</td>
	<td>
		<select style="width: 220px;" name="partner.partner_nation">
		   <option value="${partner.partner_nation }">
		   ${partner.partner_nation }
		   </option>
			<option value="汉族">
				汉族
			</option>
	...
	<td class="t-title">户籍地址:</td>
    <td><input class="t-text" type="text" id="addr" name="partner.partner_addr" value="${partner.partner_addr }"/><span class="required">*</span></td>
	</tr>
	<tr>
	<td class="t-title">证件号码:</td>
    <td><input class="t-text" type="text" name="partner.partner_IDCard" id="cardcode" value="${partner.partner_IDCard }"/><span class="required">*</span></td>
    <td class="t-title">国家:</td>
    <td>
    	<input class="t-text" type="text"  id="country" name="partner.partner_country" value="${partner.partner_country }"><span class="required">*</span>
    </tr>
	<tr>
	<td class="t-title">出资总额(万元):</td>
    <td><input class="t-text" type="text" name="partner.partner_amount" id="money" value="${partner.partner_amountStr }"/><span class="required">*</span></td>
    <td class="t-title">出资时间:</td>
    <td><input class="t-text Wdate" type="text" id="time" name="partner.partner_date" value="${partner.partner_date }" onclick="WdatePicker()" readonly="readonly"/><span class="required">*</span></td>
	</tr>
	<tr>
    <td class="t-title">备注:</td>
   <td colspan="3">
	<textarea class="t-text" style="width:540px;height: 60px" rows="30" cols="30" name="partner.partner_note">${partner.partner_note }</textarea>
   </td>
   </tr>
</table>
<div class="t-but">
		    	<input type="submit"  class="but-change" value="修改"/>
		    	<input type="button"  class="but-cancel" value="取消" onclick="closeTab()"/>
            </div>
</form>
	</div>
	</body>
