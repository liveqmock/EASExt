	...
	<script type="text/javascript">
		...
		$(function() {
			...
			$('#tt').datagrid({
				...
				url:'<%=basePath%>adminAuthority/user!queryPageJson',
				title: utitle,
				  ...
				  {field:'operation',title:'操作',width:fixColumnWidth(0.5),align:'center',       
					   formatter: function(val,row) {
						   	...
						   	if(userEmail != row.username){
						   	  result = "<span><a class=\"operation-a\" href='javascript:void(0)' onclick='openUserTab(this,\"用户列表\",\"edit\","+row.id+")'>编辑</a></span>"+
...
<body>
  ...
  <div class="search-list">
    ...
    <table id = "tt" toolbar="#toolbar"></table></div>
		<input type="hidden" id="deleteURL" value="<%=request.getContextPath()%>/adminAuthority/user!updatestutas?user.id="/>
		<input type="hidden" id="addURL" value="<%=request.getContextPath()%>/adminAuthority/user!adduserjsp"/>
		<input type="hidden" id="editURL" value="<%=request.getContextPath()%>/adminAuthority/user!edit?user.id="/>
  <div id="toolbar">
