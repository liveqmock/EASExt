...
function uploadFile(){
	...
	if(aa==''){
		$.messager.alert("提示","请选择文件","info");	
 		...
 		$("#myform").ajaxSubmit({
 			type: "POST",
 			url:"<%=basePath%>/generat/generatFileAction!generationFile",
 			resetForm: false,
	...
	$(function() {
		...
		$("#resetBtn").click(function(){
		      $("#createname,#startDate,#endDate").val("");
		});
		$('#tt').datagrid({
			//下面是 datagrid的基本 配置信息 
			url:'<%=basePath%>/generat/generatFileAction!queryFileInfo',
			width: ($(window).width()-50),
			height: 'auto',
			nowrap: false,  //  是否在一行显示数据
			striped: true,   //  是否 显示 斑马线  
			fitColumns: false,  // 自动填充 列 宽
			collapsible: false,   // 是否 有滑动效果 
			...
			columns: [[
				  ...
				  {field:'CREATENAME',title:'上传人名称',width:fixColumnWidth(0.1800)},
				  {field:'CREATETIME',title:'上传时间',width:fixColumnWidth(0.2000),
				    formatter:function(val){ return val.replace("T"," "); }
				  },
				  {field:'FID',title:'操作',width:fixColumnWidth(0.2500),
					    formatter: function(val,node){
						    ...
						    "&nbsp;<a href=\"javascript:void(0)\"  onclick='downloadSourceFile("+val+")' class=\"operation-a\">下载源文件</a>";
					  	}
				  }
			]],
			//下面 定义 分页配置 ：
		 ...
		 function downloadSourceFile(id){
			var url = "<%=basePath%>/generat/generatFileAction!isExistsSourceFile?fileFid="+id;
			var jsonCount = sendAjax(url);
			if(jsonCount=="false"){
				$.messager.alert("提示","文件下载错误");
			}else{
		  		location.href="<%=basePath%>/generat/generatFileAction!downLoadSourceFile?fileFid="+id;
			}
		}
		 function downloadFile(id){
			var url = "<%=basePath%>/generat/douwnfile!isExistsFile.action?fileFid="+id;
			var jsonCount = sendAjax(url);
			if(jsonCount=="false"){
				$.messager.alert("提示","文件下载错误");
			}else{
		  		location.href="<%=basePath%>/generat/douwnfile.action?fileFid="+id;
			}
		}
</script>
	</head>
	<body>
		...
		<table id="abc" onkeyup="bindQuery();">
    		<tr>
                <td class="s-t">&nbsp;创建人</td>
                <td><input class="s-text" type="text" id="createname" name="createname" value=""/></td>
		...
		<div class="search-list">
			...
			<input type="hidden" id="deleteURL" value="<%=basePath%>/generat/generatFileAction!deleteFile?fileFid="/>
	</body>
</html>
