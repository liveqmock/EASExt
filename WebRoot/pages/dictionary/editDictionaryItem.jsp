...
<html>
	...
	<script type="text/javascript">
		...
		$("#itemname").addClass("easyui-validatebox").validatebox({required: true,missingMessage: '必填'});
	...
	<body>
    <div class="tableForm">
		<form action="<%=basePath%>/dictionaryItem/dictionaryItemAction!<s:if test='dictionaryitem != null'>update</s:if><s:else>insert</s:else>" method="post" id="editForm">
			<input type="hidden" id="baseid" name="dictionaryitem.baseid" value='<s:property value="dictionaryitem.baseid"/>' />
			<input type="hidden" id="id" name="dictionaryitem.id" value='<s:property value="dictionaryitem.id"/>' />
			...
			<table>
                ...
                <tr>
				 <td class="t-title" width="100">数据关键字</td>
                    <td><input class="t-text readonly"  name="dictionaryitem.itemid" value='<s:property value="dictionaryitem.itemid"/>' readonly /></td>
                </tr>  
				<tr>
				 <td class="t-title" width="100">数据值</td>
                    <td><input class="t-text" id="itemname" name="dictionaryitem.itemname" value='<s:property value="dictionaryitem.itemname"/>' /><span class="required">*</span></td>
