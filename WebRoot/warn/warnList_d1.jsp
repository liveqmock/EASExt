		...
		$(function() {
			...
			$('#tt').datagrid({
				...
				columns: [[
					...
					{field:'operation',title:'操作',width:120,sortable:true,align:'center',
					   formatter: function(val,node) {
					   		//var　url = "<%=request.getContextPath()%>/person/warn!queryDetail?waringDetail.id=" + node.id;
					   		//"<span><a rel='example_group' href='" + url+"' style='color:red'>测试</a></span>";
					   		return "<span><a href='javascript:void(0)' onclick='test(this,"+node.id+")' style='color:red'>测试</a></span>";
					   }
				   }
				]],
				queryParams: {//传递参数的方法
				...
				//onSelect:function(rowIndex,rowData){
					//var　url = "<%=request.getContextPath()%>/person/warn!queryDetail?waringDetail.id=" + rowData.id;
					//window.showModalDialog(url,window,"dialogWidth:800px;dialogHeight:400px;");
				//}
			});
			//下面这个方法 用于 配置  分页的信息 
			displayMsg($('#tt'));
