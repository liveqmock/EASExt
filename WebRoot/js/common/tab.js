/* FireFox resize workaround start begin */  
var ffStartWidth = $(window).width();  
var frameWidth = window.screen.width-250;
/**
 * 火狐添加自适应监听（window.onresize不支持）
 * @return
 */
function doFirefoxResize() {  
    var resizeWidth = $(window).width(); 
    //$(".datagrid-body table").css({width:"100%"});
	//$(".datagrid-header-inner table").css({width:"100%"});
	//$(".datagrid-header-inner").css({width:"100%"});
    if (resizeWidth != ffStartWidth) {  
        ffStartWidth = resizeWidth;  
        fixWidth();  
    }  
    setTimeout("doFirefoxResize();", 800);  
}  
...
//	var href = getTopWin().$("#easyuiTheme").attr('href');
//	$("#easyuiTheme").attr('href', href)
//	$("#queryTab,#viewTab").css({width:frameWidth});
	if(document.all){//ie
	//if($.browser.msie){//ie
		doFirefoxResize();
...
function createFrame(url) {
	var s = '<iframe id="frameWrap" scrolling="auto" frameborder="0" src="'+url+'" style="width:100%;" height="100%"></iframe>';
	return s;
}

function iFrameHeight() {
    var ifm= document.getElementById("frameWrap");
    var subWeb = document.frames ? document.frames["frameWrap"].document:ifm.contentDocument;
...
//投三花名册,导入时提示异常行号、批量修改时用
function openDataTab(btn,parentTitle,type,id){
	var me = $(btn);
	var url,title=parentTitle;
	switch(type){
	   case 'viewdata':
		   ...
		   break;
	   case 'manyedit':
		   url = $("#manyeditURL").val()+id;	title+="_批量修改";
		   break;
	}
	addTab(title,url);
}
