/**
 * my97 - jQuery EasyUI Licensed under the GPL:
 * http://www.gnu.org/licenses/gpl.txt Copyright 2013 小雪转中雪 [
 * caoguanghuicgh@163.com ] [http://www.easyui.info]
 */
(function($) {
	// 如果没有my97控件，则添加之。
	(function() {
	...
	function bindEvents(target) {
		...
		if (!opts.disabled) {
			...
			arrow.bind("click.my97", function() {
				// TODO 触发my97
					WdatePicker($.extend( {}, opts, {
						el : opts.id
					}));
				}).bind('mouseenter.my97', function(e) {
	...
	function setDisabled(target, disabled) {
		...
		if (disabled) {
			...
			arrow.unbind('mouseenter.my97').unbind('mouseout').bind(
					'mouseenter.my97', function(e) {
						this.addClass('my97-arrow-hover');
					}).bind('mouseenter.my97', function(e) {
				this.removeClass('my97-arrow-hover');
			});
	...
	$.fn.my97 = function(options, param) {
		...
		return this.each(function() {
			...
			if (state) {
				...
				opts = $.extend( {}, $.fn.my97.defaults, $.fn.my97
	 ...
	 * 
	 * @param {document
	 *            object} target 承载my97的输入框
	 * @return {object} 属性列表
	 */
	$.fn.my97.parseOptions = function(target) {
		var t = $(target);
		return $.extend( {}, $.fn.validatebox.parseOptions_1(target),
				$.fn.validatebox.parseOptions(target, [ "width", "height",
